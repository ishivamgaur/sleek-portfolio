...
export default function Hero() {
  ...
  return (
    <section className="relative w-full overflow-hidden pb-4">
      {/* Banner */}
      <div className={`h-48 md:h-64 w-full relative transition-colors duration-500 ${bannerError ? 'bg-gradient-to-b from-slate-800 to-black' : 'bg-muted/30'}`}>
        {!bannerError && (
          <img 
            src="/banner.jpg" 
            alt="Banner" 
      ...
      <div className="px-4 pb-4">
        ...
        <div className="flex justify-between items-start -mt-12 md:-mt-16 mb-4 relative z-10">
          ...
          <div className="pt-10 md:pt-20 flex items-center gap-3">
            ...
            <Tooltip>
              <TooltipTrigger 
                render={<a href="https://instagram.com" target="_blank" rel="noopener noreferrer" />}
                className="p-2 rounded-md border border-border/60 hover:bg-muted/50 transition-colors text-foreground/80 hover:text-foreground flex items-center justify-center"
              >
                <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
            ...
            <Tooltip>
              <TooltipTrigger 
                render={<a href="https://twitter.com" target="_blank" rel="noopener noreferrer" />}
                className="p-2 rounded-md border border-border/60 hover:bg-muted/50 transition-colors text-foreground/80 hover:text-foreground flex items-center justify-center"
              >
                <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
          ...
          >
            ...
            >
              ...
              <div className="absolute top-6 inset-x-4 flex gap-1.5 z-20">
                {stories.map((_, idx) => (
                  <div key={idx} className="h-1 flex-1 bg-white/20 rounded-md overflow-hidden">
                    <div 
              ...
              >
                ...
                {currentStory?.imageUrl && (
                  <div className="absolute bottom-12 inset-x-0 p-6 bg-gradient-to-t from-black/80 to-transparent text-white">
                    <p className="text-lg font-medium leading-relaxed">{currentStory.content}</p>
                  </div>
                )}
              </div>
              ...
              {stories.length > 1 && (
                <>
                  <button onClick={(e) => { e.stopPropagation(); handlePrev(); }} className="absolute left-4 top-1/2 -translate-y-1/2 p-2 bg-black/20 hover:bg-black/40 rounded-md text-white backdrop-blur-sm z-30 opacity-0 md:opacity-100 transition-opacity" disabled={activeStoryIdx === 0}>
                    <ChevronLeft className="w-6 h-6" />
                  </button>
                  <button onClick={(e) => { e.stopPropagation(); handleNext(false); }} className="absolute right-4 top-1/2 -translate-y-1/2 p-2 bg-black/20 hover:bg-black/40 rounded-md text-white backdrop-blur-sm z-30 opacity-0 md:opacity-100 transition-opacity">
