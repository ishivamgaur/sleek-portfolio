...
export function ExperienceItem({ story, index, showTech = true }: ExperienceItemProps) {
  return (
    <motion.div
      ...
      <div className="flex flex-col space-y-3">
        ...
        <div className="flex flex-col space-y-1.5">
          <div className="flex items-start justify-between gap-2">
            <h3 className="text-[16px] md:text-[17px] font-bold tracking-tight text-foreground leading-tight min-w-0 flex-1">{story.company}</h3>
            <span className="text-[10px] md:text-[11px] text-muted-foreground/80 font-mono font-bold uppercase tracking-wider shrink-0 mt-0.5">
              {formatDate(story.startDate)} — {story.date === 'Current' ? 'Current' : formatDate(story.date)}
            </span>
          </div>
          
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-1 md:gap-4">
            <p className="text-[13px] md:text-[14px] font-medium text-foreground/60">{story.role}</p>
            ...
            <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground/70">
              ...
              <span className="md:text-[10px] md:uppercase md:tracking-tight md:font-semibold">{story.type}</span>
            </div>
          </div>
        </div>
        ...
        <ul className="list-disc list-outside ml-4 space-y-2">
          {story.content.split('.').filter(line => line.trim()).map((bullet, i) => (
            <li key={i} className="text-muted-foreground leading-relaxed text-[14px]">
              {bullet.trim()}.
            </li>
          ))}
        </ul>
...
function TechDock({ tech }: { tech: string[] }) {
  ...
  return (
    <div className="flex flex-wrap gap-2.5 items-center" onMouseLeave={() => setHoveredIndex(null)}>
      {tech.map((t, i) => {
        const isHovered = hoveredIndex === i;
        const isSomethingHovered = hoveredIndex !== null;
        
        return (
          <div key={i} onMouseEnter={() => setHoveredIndex(i)} className="relative">
            <Tooltip>
